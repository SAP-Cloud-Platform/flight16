...
FUNCTION /DMO/FLIGHT_TRAVEL_CALC_PRI_16.
*"----------------------------------------------------------------------
*"*"Local Interface:
*"  IMPORTING
*"     REFERENCE(IT_FLIGHT) TYPE  /DMO/IF_FLIGHT_LEGACY16=>TT_FLIGHT
*"       OPTIONAL
*"----------------------------------------------------------------------
  TYPES BEGIN OF ty_seats_occupied.
  INCLUDE TYPE /dmo/flight16.
  TYPES   distance       TYPE /dmo/connecti_16-distance.
  TYPES   distance_unit  TYPE /dmo/connecti_16-distance_unit.
  ...
  IF it_flight IS SUPPLIED.
    SELECT FROM /dmo/booking16 AS booking
      JOIN     /dmo/connecti_16 AS connection
                 ON  booking~carrier_id    = connection~carrier_id
                 AND booking~connection_id = connection~connection_id
      JOIN     /dmo/flight16 AS flight
